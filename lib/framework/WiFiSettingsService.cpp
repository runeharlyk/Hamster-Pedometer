...
void WiFiSettingsService::loop() {
  ...
  if (!_lastRssiUpdate ||
      (unsigned long)(currentMillis - _lastRssiUpdate) >= RSSI_EVENT_DELAY) {
  ...
  if (scanResult == WIFI_SCAN_FAILED) {
    ...
    for (int i = 0; i < scanResult; ++i) {
      ...
      WiFi.getNetworkInfo(i, ssid_scan, sec_scan, rssi_scan, BSSID_scan,
                          chan_scan);
      ESP_LOGV("WiFiSettingsService",
               "SSID: %s, BSSID: " MACSTR ", RSSI: %d dbm, Channel: %d",
               ssid_scan.c_str(), MAC2STR(BSSID_scan), rssi_scan, chan_scan);
      ...
      for (auto &network : _state.wifiSettings) {
        if (ssid_scan == network.ssid) {   // SSID match
          if (rssi_scan > bestNetworkDb) { // best network
            ...
            ESP_LOGV("WiFiSettingsService",
                     "--> New best network SSID: %s, BSSID: " MACSTR "",
    ...
    if (_state.priorityBySignalStrength == false) {
      for (auto &network : _state.wifiSettings) {
        if (network.available == true) {
          ESP_LOGI("WiFiSettingsService",
                   "Connecting to first available network: %s",
                   network.ssid.c_str());
      ...
      ESP_LOGI("WiFiSettingsService",
               "Connecting to strongest network: %s, BSSID: " MACSTR " ",
...
void WiFiSettingsService::configureNetwork(wifi_settings_t &network) {
  if (network.staticIPConfig) {
    ...
    WiFi.config(network.localIP, network.gatewayIP, network.subnetMask,
                network.dnsIP1, network.dnsIP2);
  } else {
    // configure for DHCP
  ...
  WiFi.begin(network.ssid.c_str(), network.password.c_str(), network.channel,
             network.bssid);
  // WiFi.begin(network.ssid.c_str(), network.password.c_str());

#if CONFIG_IDF_TARGET_ESP32C3
  WiFi.setTxPower(
      WIFI_POWER_8_5dBm); // https://www.wemos.cc/en/latest/c3/c3_mini_1_0_0.html#about-wifi
#endif
}
...
void WiFiSettingsService::onStationModeDisconnected(WiFiEvent_t event,
                                                    WiFiEventInfo_t info) {
  WiFi.disconnect(true);
}
void WiFiSettingsService::onStationModeStop(WiFiEvent_t event,
                                            WiFiEventInfo_t info) {
  if (_stopping) {
    _lastConnectionAttempt = 0;
    _stopping = false;
