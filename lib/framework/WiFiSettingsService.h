...
public:
  ...
  static StateUpdateResult update(JsonObject &root, WiFiSettings &settings) {
    settings.hostname =
        root["hostname"] | SettingValue::format(FACTORY_WIFI_HOSTNAME);
    ...
    if (root["wifi_networks"].is<JsonArray>()) {
      ...
      for (auto wifiNetwork : wifiNetworks) {
        ...
        // Check if SSID length is between 1 and 31 characters and password
        // between 0 and 64 characters
        if (wifi["ssid"].as<String>().length() < 1 ||
            wifi["ssid"].as<String>().length() > 31 ||
          ...
          if (IPUtils::isNotSet(wifiSettings.dnsIP1) &&
              IPUtils::isSet(wifiSettings.dnsIP2)) {
            wifiSettings.dnsIP1 = wifiSettings.dnsIP2;
            wifiSettings.dnsIP2 = INADDR_NONE;
          }

          // Turning off static ip config if we don't meet the minimum
          // requirements of ipAddress, gateway and subnet. This may change to
          // static ip only as sensible defaults can be assumed for gateway and
          // subnet
          if (wifiSettings.staticIPConfig &&
              (IPUtils::isNotSet(wifiSettings.localIP) ||
               IPUtils::isNotSet(wifiSettings.gatewayIP) ||
