...
#define EXECUTE_EVERY_N_MS(n, code)                                            \
  do {                                                                         \
    ...
    if (UNIQUE_VAR(lastExecution_) == 0 ||                                     \
        currentMillis - UNIQUE_VAR(lastExecution_) >= n) {                     \
      code;                                                                    \
...
#define TIME_IT(code)                                                          \
  {                                                                            \
    ...
    if (time_it_elapsed < 1000) {                                              \
      ...
      ESP_LOGI("Time It", "Time elapsed: %lu milliseconds",                    \
               time_it_elapsed / 1000);                                        \
    } else {                                                                   \
      ESP_LOGI("Time It", "Time elapsed: %.2f seconds",                        \
               time_it_elapsed / 1000000.0);                                   \
    }                                                                          \
  }
