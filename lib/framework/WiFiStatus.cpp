...
void WiFiStatus::begin() {
  _server->on(
      WIFI_STATUS_SERVICE_PATH, HTTP_GET,
      _securityManager->wrapRequest(
          std::bind(&WiFiStatus::wifiStatus, this, std::placeholders::_1),
          AuthenticationPredicates::IS_AUTHENTICATED));

  ESP_LOGV("WiFiStatus", "Registered GET endpoint: %s",
...
void WiFiStatus::onStationModeConnected(WiFiEvent_t event,
                                        WiFiEventInfo_t info) {
  ESP_LOGI("WiFiStatus", "WiFi Connected.");
}

void WiFiStatus::onStationModeDisconnected(WiFiEvent_t event,
                                           WiFiEventInfo_t info) {
  ESP_LOGI("WiFiStatus", "WiFi Disconnected. Reason code=%d",
           info.wifi_sta_disconnected.reason);
}

void WiFiStatus::onStationModeGotIP(WiFiEvent_t event, WiFiEventInfo_t info) {
  ESP_LOGI("WiFiStatus", "WiFi Got IP. localIP=%s, hostName=%s",
