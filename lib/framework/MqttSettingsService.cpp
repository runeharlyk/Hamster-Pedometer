...
extern const uint8_t rootca_crt_bundle_start[] asm(
    "_binary_src_certs_x509_crt_bundle_bin_start");

/**
 * Retains a copy of the cstr provided in the pointer provided using dynamic
 * allocation.
 *
 * Frees the pointer before allocation and leaves it as nullptr if cstr ==
 * nullptr.
 */
...
MqttSettingsService::MqttSettingsService(PsychicHttpServer *server, FS *fs,
    ...
    : _server(server), _securityManager(securityManager),
      _httpEndpoint(MqttSettings::read, MqttSettings::update, this, server,
                    MQTT_SETTINGS_SERVICE_PATH, securityManager),
      ...
      _reconfigureMqtt(false), _mqttClient(), _lastError("None") {
...
void MqttSettingsService::begin() {
  WiFi.onEvent(std::bind(&MqttSettingsService::onStationModeDisconnected, this,
                         std::placeholders::_1, std::placeholders::_2),
               WiFiEvent_t::ARDUINO_EVENT_WIFI_STA_DISCONNECTED);
  WiFi.onEvent(std::bind(&MqttSettingsService::onStationModeGotIP, this,
                         std::placeholders::_1, std::placeholders::_2),
               WiFiEvent_t::ARDUINO_EVENT_WIFI_STA_GOT_IP);
  _mqttClient.onConnect(std::bind(&MqttSettingsService::onMqttConnect, this,
                                  std::placeholders::_1));
  _mqttClient.onDisconnect(std::bind(&MqttSettingsService::onMqttDisconnect,
                                     this, std::placeholders::_1));
  _mqttClient.onError(std::bind(&MqttSettingsService::onMqttError, this,
                                std::placeholders::_1));
...
bool MqttSettingsService::isEnabled() { return _state.enabled; }

bool MqttSettingsService::isConnected() { return _mqttClient.connected(); }
...
PsychicMqttClient *MqttSettingsService::getMqttClient() { return &_mqttClient; }

String MqttSettingsService::getLastError() { return _lastError; }
...
void MqttSettingsService::onConfigUpdated() { _reconfigureMqtt = true; }

void MqttSettingsService::onStationModeGotIP(WiFiEvent_t event,
...
void MqttSettingsService::onStationModeDisconnected(WiFiEvent_t event,
                                                    WiFiEventInfo_t info) {
  if (_state.enabled) {
  ...
  if (_state.enabled && WiFi.isConnected()) {
    ...
    if (_state.username.length() > 0) {
      _mqttClient.setCredentials(
          ...
          retainCstr(_state.password.length() > 0 ? _state.password.c_str()
                                                  : nullptr,
                     &_retainedPassword));
    } else {
    ...
    _mqttClient.setClientId(
        retainCstr(_state.clientId.c_str(), &_retainedClientId));
