...
template <class T> class FSPersistence {
public:
  FSPersistence(JsonStateReader<T> stateReader,
      ...
      : _stateReader(stateReader), _stateUpdater(stateUpdater),
        _statefulService(statefulService), _filePath(filePath),
        _updateHandlerId(0) {
    enableUpdateHandler();
  }
  ...
  void readFromFS() {
    ...
    // If we reach here we have not been successful in loading the config and
    // hard-coded defaults are now applied. The settings are then written back
    // to the file system so the defaults persist between resets. This last step
    // is required as in some cases defaults contain randomly generated values
    // which would otherwise be modified on reset.
    applyDefaults();
    writeToFS();
  }
