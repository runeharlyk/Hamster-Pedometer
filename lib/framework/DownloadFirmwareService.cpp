...
void update_progress(int currentBytes, int totalBytes) {
  ...
  if (progress > previousProgress) {
    ...
    JsonObject jsonObject = doc.as<JsonObject>();
    _socket->emitEvent(EVENT_DOWNLOAD_OTA, jsonObject);
    ESP_LOGV(TAG, "HTTP update process at %d of %d bytes... (%d %%)",
             currentBytes, totalBytes, progress);
  }
...
esp_err_t DownloadFirmwareService::downloadUpdate(PsychicRequest *request,
                                                  JsonVariant &json) {
